...
public final class DRM {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Encoding encoding = createEncoding("Multiple FMP4 with DASH and HLS",
                    "Encoding with multiple fMP4 muxings");
            ...
            for (H264VideoConfiguration videoConfiguration : videoConfigurations) {
                ...
                Fmp4Muxing videoMuxing = createFmp4Muxing(encoding, output, videoConfiguration.getHeight().toString(), videoStream);
                createDrmConfig(encoding, videoMuxing, output, "video",cmd);
                
            }

            Fmp4Muxing audioMuxing = createFmp4Muxing(encoding, output, "audio", audioStream);

            executeEncoding(encoding);

       
            createDrmConfig(encoding, audioMuxing, output, "audio",cmd);
    ...
    private static CencDrm createDrmConfig(Encoding encoding, Muxing muxing, Output output, String outputPath,CommandLine cmd)
        ...
        return bitmovinApi.encoding.encodings.muxings.fmp4.drm.cenc.create(encoding.getId(), muxing.getId(), cencDrm);
    }
    ...
    private static Options createOptions() {
        ...
        Option ck = new Option("ck", "cenc-key", true, "CENC Key");
        inputPath.setRequired(false);
        options.addOption(ck);

        Option ckid = new Option("ckid", "cenc-key-id", true, "CENC Key id");
        inputPath.setRequired(false);
        options.addOption(ckid);

        Option wvp = new Option("wvp", "widevine-pssh", true, "Widevine PSSH");
        inputPath.setRequired(false);
        options.addOption(wvp);

        Option fiv = new Option("fiv", "fairplay-iv", true, "Fairplay IV");
        inputPath.setRequired(false);
        options.addOption(fiv);

        Option furi = new Option("furi", "fairplay-uri", true, "Fairplay URI");
        inputPath.setRequired(false);
        options.addOption(furi);
